...
<?php
	...
	if(isset($_POST['edit'])){
		...
		// chèn vào
		echo $sql = "update `danhmuc` set ten ='$name',
									 `parent` = '$parent',
									  image = '$img' 
								 where id = ".$id;
		exec_update($sql);
		
		$sql = "update `danhmuc` set count = count + 1 where id = ".$parent;
		exec_update($sql);
						...
						<div class=" row col-md-8">
							...
							<select class="col-md-8" name="parent">
								...
								<?php foreach($categories as $key){?>
									<option value="<?= $key["id"]?>" <?php if($category['parent'] == $key["id"]) echo "selected"?>><?= $key["ten"]?></option>
								<?php }?>
							</select>
						</div>
						<div class="form-group col-md-12">
							<label class="col-md-4"> Tên danh mục</label>
							<input class="col-md-8" name="name" value="<?= $category['ten']?>" required>
						</div>
						<div class="form-group col-md-12">
